...
- Delivered in person (in-person at gym, in-home, or outdoor)
- At least 50% of the scheduled time completed with the client present

**Cancellation after the 24-hour window = still a session.** If a client cancels with less than 24 hours' notice and you're available and prepared, that counts as delivered.

**No-show from you = no pay.** If you miss a session without an approved swap, it doesn't count.

---

## The Five Session Categories (pick one per row)

Sessions must be classified by **location + format**:

| Category | What this means |
|---|---|
| **Gym - 1:1** | One-on-one session at Point One Fitness (Russell Road) |
| **Gym - Group** | Group session (2–6 clients) at Point One Fitness |
| **In-Home - 1:1** | One-on-one session at the client's residence |
| **In-Home - Group** | Group session at a client's residence or shared home |
| **Outdoor/Virtual** | Outdoor session (park, trail, etc.) or virtual (video call) |

> If you're unsure which category to use, default to **Gym - 1:1** for anything at Point One Fitness and ask Eric.
...
- "Skipped planned workout due to client fatigue — did mobility instead"
- "Group of 4 — completed warmup + resistance band circuit, 2 clients hit new reps"
- "Client arrived 10 min late, shortened session to 40 min — completed planned routine"
...
## Frequently Overlooked Details (Gotchas)

1. **No calendar match = no pay.** If the session isn't on the calendar, Eric can't verify it. Always check the calendar before delivering a session.
...
Ask Eric. Do not email — text or call. If you're ever unsure whether something counts as a valid session, **ask before you deliver**, not after.
